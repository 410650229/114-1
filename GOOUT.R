...
# 假設您的資料框名稱為 movies_final

# 確認結果
head(movies_final$fir_year)
...
write.csv(Name_analysis, "Name_analysis.csv", row.names = FALSE)
...
grade_distribution <- movies_final %>%
  ...
  spread(key = grade, value = count, fill = 0) # 將分級展平為寬表格

# 計算從第27到第59個變數的主題分布
topic_analysis <- movies_final %>%
  select(Name, 27:59) %>% # 選擇第27到第59個變數
...
combined_analysis <- media_type_analysis %>%
  left_join(topic_analysis, by = "Name")
...
analysis_summary <- list(
  ...
  combined_analysis = combined_analysis # 合併結果
)
...
combined_analysis <- combined_analysis %>%
  mutate(
    represent = apply(select(., 5:37), 1, function(x) names(x)[which.max(x)])
  )
